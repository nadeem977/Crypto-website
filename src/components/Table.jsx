...
const Table = () => {
  ...
  const { cryptodata, currency, setCoinsID, setLocalsave, errors } = useContext(CryptoContext);
  const error = () => toast(errors);
  ...
  let storedData = localStorage.getItem("coins");
  let data = [];
  ...
  const existingCoinIndex = data.findIndex((coin) => coin.id === item.id);
  if (existingCoinIndex !== -1) {
    data.splice(existingCoinIndex, 1);
  }

  data.push(item);
  localStorage.setItem("coins", JSON.stringify(data));
  setLocalsave(data);
  setActive(data.map((coin) => coin.id));
  };

  const isCoinActive = (coinId) => {
  return active.includes(coinId);
  };
  ...
  return (
    <>
                        ...
                        <button className="outline-0 border-0 bg-none cursor-pointer">
                          <svg
                           className={`w-[1.5rem] ml-1.5 ${
                            isCoinActive(item.id) ? "fill-cyan" : "fill-gray-100"
                          }`}
                            width="30"
                            height="30"
                            viewBox="0 0 30 30"
      ...
      <div className="pagin flex items-center justify-between mt-4 capitalize h-[2rem]">
        <span>
          Data provided by{" "}
          <a
            className="text-cyan"
