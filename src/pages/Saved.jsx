...
const Saved = () => {
  ...
  const { localsave, setLocalsave ,currency,setCoinsID} = useContext(CryptoContext);
  ...
  const handleDelete = (index) => {
    const updatedData = [...localsave];
    updatedData.splice(index, 1);
    setLocalsave(updatedData);
    localStorage.setItem('coins', JSON.stringify(updatedData));
  };
  useEffect(() => {
    const savedData = localStorage.getItem('coins');
    if (savedData) {
      setLocalsave(JSON.parse(savedData));
    }
  },[]);

  useEffect(() => {
    if (localsave) {
      localStorage.setItem('coins', JSON.stringify(localsave));
    }
  },[localsave]);
  ...
  return (
      ...
      <>
      <div className="save-div w-[80%] flex flex-col mt-9 border border-gray-100 rounded">
      {Array.isArray(localsave) && localsave.length >0 ? (
        <table className="w-full table-auto table-responsive">
          <thead className="capitalize text-base text-gray-100 font-medium border-b border-gray-100">
            <tr>
              ...
              <th className="py-1">change parcentage price 24h</th>
            </tr>
          </thead>
          <tbody>
                ...
                <tr
                  ...
                  <td className="py-4 flex items-center uppercase">
                    <button className="outline-0 border-0 bg-none cursor-pointer">
                      <svg
                        className="w-[1.5rem] ml-1.5 fill-cyan"
                        width="30"
                        height="30"
                        viewBox="0 0 30 30"
                        fill="cyan"
                    ...
                    <img
                      src={item.image}
                      alt={item.name}
                      className="w-[1.2rem] h-[1.2rem] mx-1.5 cursor-pointer"
                      onClick={() => coinsID(item.id)}/>
                    <span onClick={() => coinsID(item.id)} className='cursor-pointer'>{item.symbol}</span>
                  </td>
                  <td
                  ...
                  <td className="py-4">{item.market_cap_rank}</td>
                  <td
                    className={
                      item.price_change_24h > 0
                        ? "text-green py-4"
                        : "text-red py-4"}>
                    {Number(item.price_change_24h).toFixed(2)}
                  </td>
                  <td
                    className={
       ...
       <div className='min-h-[60vh] flex justify-center items-center'>
        <h2 className='text-lg'>  There is no data to display!</h2>
       </div>
      )}
    </div>
  
     <Models  modalcoinID={modalcoinID}/>
      </>
  );
};
